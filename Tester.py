...
if __name__ == '__main__':
    print('=' * 130)
    print('Test cases' + ' ' * (40 - len('Test cases')) + 'Task-1' + ' ' * (30 - len('Task-1')) + 'Task-2' + ' ' * (
                30 - len('Test-2')) + 'Task-3')
    print('-' * 130)
    passedScore = 0
    fileGraph = 'input\\basicGraph.txt'
    fileCamera = 'input\\camera.txt'
    fileToll = 'input\\toll.txt'
    fileService = 'input\\service.txt'
    errorType = ['', 'Graph loading failed', 'Program crashed', 'Return type mismatch', 'Path length incorrect',
                 'Path incorrect']
    testType = ['', 'normal and detour same, but safe', 'ditto', 'Shortest path is already safe',
                'Source and sink same', 'Normal and safe same, but detour', 'Blocked path', 'All different',
                'Detour is source', 'Camera is in source', 'NO detour and safe', 'No detour']
    ...
    for i in range(1, 11):
        ...
        if error is not None:
            string = 'Test:' + testType[i] + ' ' * (40 - len('Test:' + testType[i])) + errorType[error] + ' ' * (
                        30 - len(errorType[error]))
        else:
            string = 'Test:' + testType[i] + ' ' * (40 - len('Test:' + testType[i])) + 'PASSED' + ' ' * (
                        30 - len('PASSED'))
